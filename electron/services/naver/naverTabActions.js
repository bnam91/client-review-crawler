...
export async function setSortOption(page, sortOption) {
  ...
  console.log(`[NaverTabActions] 🔧 정렬 옵션을 설정합니다... (받은 값: ${sortOption}, 변환 후: ${sortNum}, ${sortNames[sortNum] || '알 수 없음'})`);
...
export async function clickReviewOrQnATab(page, collectionType, sortOption = 0) {
  ...
  console.log(`[NaverTabActions] 🔍 ${tabLabel} 탭을 찾는 중... (받은 sortOption: ${sortOption}, 변환 후: ${sortNum}, ${sortNames[sortNum] || '알 수 없음'})`);
  ...
  try {
    ...
    if (isActive) {
      ...
      if (collectionType === 0) {
        const sortNames = ['랭킹순', '최신순', '평점낮은순'];
        console.log(`[NaverTabActions] 정렬 옵션 적용 전 - sortOption: ${sortOption}, 변환 후: ${sortNum} (${sortNames[sortNum] || '알 수 없음'})`);
